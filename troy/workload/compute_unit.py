...
class ComputeUnit (sa.Attributes) :
    """
    The `ComputeUnit` class represents a element of work to be performed on
    behalf of an application, and is part of a workload managed by Troy.

    ComputeUnit instances are created and owned by the :class:`Workload` class
    they are part of -- only that class should change its composition and state.
    ...
    def __init__ (self, descr) :
        """
        Create a new workload element, aka ComputeUnit, according to the description..
        ...
        if  not 'tag' in descr :
            raise ValueError ("no 'tag' in ComputeUnitDescription")
