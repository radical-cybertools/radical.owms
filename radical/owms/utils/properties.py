...
class Properties (saga.Attributes) :
    """
    Several radical.owms classes benefit from somewhat richer than default python
    ...
    For that to work, radical.owms needs to declare which properties are eligible for
    those callbacks::

        class radical.owms.workload (radical.owms.utils.Properties) :

            self.register_property ('state')
            ...

    From that point on, any plugin, or any thread within radical.owms, can set the
    ...
    use::
        ...
        tasks[i].state = radical.owms.FAILED
    ...
    It is costly for radical.owms to frequently update all possible information from
    ...
    accessed.  For that, radical.owms needs again to explicitly register properties
    ...
    def __init__ (self, inits={}) :
        """
        set up the radical.owms property interface -- allow normal properties (set as
