...
class _Unit (object) :
    ...
    def __init__ (self, descr) :
        ...
        radical.owms._logger.debug ("new     unit %s" % (self.id))
    ...
    def run (self) :
        ...
        radical.owms._logger.debug ("running unit %s (%s)" % (self.id, command))
    ...
    def get_state (self) :
        ...
        if  self._state in [_RUNNING] :
            ...
            if  self._proc.poll () is not None :
                ...
                radical.owms._logger.debug ("unit finished: %s" % self.id)
                radical.owms._logger.debug ("     exitcode: %s" % self.retval)
                radical.owms._logger.debug ("     output  : %s" % out[-1024:])
                radical.owms._logger.debug ("     error   : %s" % err[-1024:])
    ...
    def cancel (self) :
        ...
        radical.owms._logger.debug ("cancel  unit %s" % (self.id))
...
class _Pilot (object) :
    ...
    def __init__ (self, pilot) :
        ...
        radical.owms._logger.debug ("new     pilot %s" % (self.id))
    ...
    def run (self) :
        ...
        self.state = _RUNNING
        self.start = time.time()
        radical.owms._logger.debug ("run     pilot %s" % (self.id))
    ...
    def submit_unit (self, descr) :
        ...
        radical.owms._logger.debug ("submit  unit %s to pilot %s" % (unit.id, self.id))
    ...
    def cancel (self) :
        ...
        radical.owms._logger.debug ("cancel  pilot %s" % (self.id))
        self.state = _CANCELED
...
class PLUGIN_CLASS (radical.owms.PluginBase):
    ...
    **Configuration Options:**  None
    ...
    def provision (self, overlay) :
        """
        We basically do just enough to make radical.owms think that we did something
        ...
        for pid in overlay.pilots.keys() :
            ...
            radical.owms._logger.info ('overlay  provision: provision pilot  %s : %s ' % (pilot, p))
