...
class PLUGIN_CLASS (radical.owms.PluginBase):
    ...
    * `coordination_url`: the redis URL to be used by RADICAL-Pilot.  The environment
    ...
    def dispatch (self, workload, overlay) :
        ...
        for tid in workload.tasks.keys () :
            ...
            for uid in task.units.keys () :
                ...
                pilot      = radical.owms.Pilot (self.session, pilot_id, _instance_type='radical.pilot')
                radical.owms._logger.info ('workload dispatch : dispatch %-18s to %s' \
    ...
    def unit_reconnect (self, native_id) :
        ...
        This is what is getting called on radical.owms.Unit._get_instance, if that
        radical.owms.Unit doesn't have that instance anymore...
        """

        radical.owms._logger.debug ("reconnect to radical.pilot cu %s" % native_id)
    ...
    def unit_get_info (self, unit) :
        ...
        if  'state' in info :
            radical.owms._logger.debug ('radical.pilot level cu state: %s' % info['state'])
      ...
      # print 'unit_get_info: %s' % info
        ...
        if  info['state'] == FAILED :
            radical.owms._logger.error ('CU %s failed' % unit.id)
            radical.owms._logger.info ('log: \n----\n%s\n---\n' % info['log'])
            radical.owms._logger.info ('stderr: \n----\n%s\n---\n' % info['stderr'])
            radical.owms._logger.info ('stdout: \n----\n%s\n---\n' % info['stdout'])
