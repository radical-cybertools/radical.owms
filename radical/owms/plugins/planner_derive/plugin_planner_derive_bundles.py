...
class PLUGIN_CLASS (radical.owms.PluginBase):
    ...
    * `guard`: documentation for guard
    ...
    def init(self):
        ...
        radical.owms._logger.debug ("init plugin %s (bundles)" % self.name)
    ...
    def init_bundles(self):
        ...
        radical.owms._logger.info('Initializing Bundle Manager')

        self.bm = BundleManager()

        cg = self.session.get_config ('radical:owms:bundle')
        finished_job_trace = cg['finished_job_trace']

        # FIXME: not sure if the new resource config contains all needed data...
        for sect in self.session.get_config ('radical:owms:resources'):
    ...
    def derive_overlay(self, workload, guard=LOWER_LIMIT):
        ...
        ovl_descr = radical.owms.OverlayDescription (
            {
                # Ask for as many pilots as tasks
                'cores' : cores,
            })

        radical.owms._logger.info('planner derive ol: derive overlay for workload: '
                          '%s' % ovl_descr)
