    ...
    * `concurrency`: percentage of concurrent tasks in the workload.  
    ...
    def derive_overlay(self, workload):
        ...
        if  len(workload.partitions) > 1 :
            radical.owms._logger.warning ("workload is already partitioned, ignore concurrency setting")
            ...
            radical.owms._logger.info ("planner uses concurrency of %d%%" % concurrency)
            ...
            for partition_list in task_partitions :
                ...
                c_partition = radical.owms.Workload (workload.session)
                ...
                for n in range (n_concurrent, n_tasks) :
                    s_partition = radical.owms.Workload (workload.session)
            ...
            radical.owms._logger.info ("created %d workload partitions" % len(workload.partitions))
        ...
        radical.owms._logger.info ("planner  derived overlay: %s" % ovl_descr)

        print "--------------------------------------------------------------"
        for c_partition_id in workload.partitions :
            c_partition = radical.owms.WorkloadManager.get_workload (c_partition_id)
