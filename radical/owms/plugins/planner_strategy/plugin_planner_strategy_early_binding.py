...
PLUGIN_DESCRIPTION = {
    ...
    'name'        : 'early_binding', 
    'version'     : '0.1',
    'description' : 'this is the basic radical.owms strategy for executing workloads.'
  }
    ...
    **Configuration Options:** None
    ...
    def execute (self, workload_id, planner, overlay_mgr, workload_mgr) :
        ...
        try :
            ...
            # first, we set the 'AUTO' plugins to the radical.owms defaults.  The managers
            # would do that anyways, but we want to explicitly make this part of the
            # stratey.

            radical.owms._logger.info ('apply early binding strategy to workload %s!' % workload_id)
            ...
            for partition_id in workload.partitions[:1] :
                ...
                radical.owms._logger.info  ("dispatch workload partition 0 (%s)" % partition_id)
                ...
                # of the current partition.  This only works because
                # radical.owms does
            ...
            for partition_id in workload.partitions[:1] :
                ...
                if partition.state == radical.owms.DONE :
                    radical.owms._logger.info  ("partition %s done" % partition.id)
                else :
                    radical.owms._logger.error ("partition %s failed - abort" % partition.id)
            ...
            for partition_id in workload.partitions[1:] :
                ...
                radical.owms._logger.info  ("running workload partition %s" % partition_id)
                partition = radical.owms.WorkloadManager.get_workload (partition_id)
                ...
                if partition.state == radical.owms.DONE :
                    radical.owms._logger.info  ("partition done")
                else :
                    radical.owms._logger.error ("partition failed - abort")
            ...
            radical.owms._logger.info ("all partition done (%s)" % workload.state)

            radical.owms._logger.warn ("shutting down workload: %s" % workload.id)
            workload_mgr.cancel_workload (workload.id)

            radical.owms._logger.warn ("shutting down overlay: %s" % overlay.id)
            overlay_mgr.cancel_overlay (overlay.id)

            session = planner.session
            if  session.cfg.get ('radical_owms_timing') == 'store' :
                tgt = session.cfg.get ('radical_owms_timing_db')

                if  not tgt :
                    radical.owms._logger.critical ("cannot store timings, no radical_owms_timing_db")
                else :
            ...
            radical.owms._logger.critical ("strategy execution failed: %s" % e)
            import traceback
            traceback.print_exc()
