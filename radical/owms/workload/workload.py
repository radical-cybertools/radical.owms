...
class Workload (tu.Properties, tu.Timed) :
    """
    The `Workload` class represents a workload which is managed by radical.owms.  It
...
relation    * *Translation:* A workload is inspected, and its tasks are translated into 
    ...
    * *Dispatching:* A scheduled workload is dispatched to the active entities
    ...
    feedback loops are considered out-of-scope for radical.owms at this point, so that
    state transitions are considered irreversible.  

    Workload Partitions
    -------------------

    A radical.owms Planner (or in fact any other algorithm in radical.owms) can partition
    ...
    A radical.owms workload maintains a set of partitions, and any algorithm, in
    particular the radical.owms.strategy and the workload.dispatcher plugins, may or may
    ...
    partition IDs, which can be translated to `radical.owms.Workload` instances relationvia
    `radical.owms.WorkloadManager.get_workload (workload.partitions[0])` etc.  That list
    ...
    changed in future versions of radical.owms.
    """
    ...
    def __init__ (self, session, task_descriptions=None, relation_descriptions=None) :
        ...
        tu.Timed.__init__            (self, 'radical.owms.Workload', self.id)
        self.session.timed_component (self, 'radical.owms.Workload', self.id)
        ...
        radical.owms.WorkloadManager.register_workload (self)
    ...
    def cancel (self) :
        ...
        if  self.state in [DISPATCHED] :
            ...
            radical.owms._logger.info ('cancel workload %s' % self.id)
    ...
    def add_task (self, descr) :
        ...
        for d in descr :
            ...
            if  isinstance (d, dict) :
                d = radical.owms.TaskDescription (d)

            if  not isinstance (d, radical.owms.TaskDescription) :
            ...
            task = radical.owms.Task (self.session, descr=d, _workload=self)
        
            self.timed_component (task, 'radical.owms.Task', task.id)
    ...
    def add_relation (self, descr) :
        ...
        for d in descr :
            ...
            if  not isinstance (d, radical.owms.RelationDescription) :
    ...
    def get_walltime (self) :
        ...
        if  len(self.partitions) > 1 :
            for partition_id in self.partitions :
                partition = radical.owms.WorkloadManager.get_workload (partition_id)
                walltime += partition.get_walltime ()
        else :
    ...
    def get_state (self) :
        ...
        The initial stages of radical.owms cause atomic state transitions for the
        ...
        Up to then, all state transitions are under full control of radical.owms, so we
        ...
        radical.owms._logger.debug ('wl   state %-6s: %-10s %s' % (self.id, self.state, str(task_states)))
        return self.state
    ...
    def wait (self) :
        ...
        while self.state not in [radical.owms.DONE, radical.owms.FAILED,
                radical.owms.CANCELED]:
            radical.owms._logger.info ("waiting for workload (state: %s)" % self.state)
            time.sleep(1)
