...
class WorkloadManager (tu.Timed) :
    ...
    # this map is used to translate between radical.owms unit IDs and native backend
    # IDs.
    _unit_id_map = dict ()
    ...
    def __init__ (self, session) :
        ...
        tu.Timed.__init__             (self, 'radical.owms.WorkloadManager', self.id)
        self.session.timed_component  (self, 'radical.owms.WorkloadManager', self.id)
    ...
    def _init_plugins (self) :
        ...
        # radical.owms._logger.debug ("initializing workload manager (%s)" % self.plugins)
        ...
        radical.owms._logger.info ("initialized  workload manager (%s)" % self.plugins)
    ...
    def register_workload (cls, workload) :
        ...
        for workload in workloads :
            ...
            if  not isinstance (workload, radical.owms.Workload) :
                raise TypeError ('expected radical.owms.Workload instance, not %s' % type(workload))
    ...
    def get_workload (cls, workload_id, _manager=None) :
        ...
        if  _manager :
            _manager.timed_component (workload, 'radical.owms.Workload', workload_id)
    ...
    def native_id_to_unit_id (cls, native_id) :
        ...
        for radical.owms_id in cls._unit_id_map :
            if  native_id == cls._unit_id_map[radical.owms_id] :
                return radical.owms_id
    ...
    def parse_workload (self, workload_description) :
        ...
        try :
            ...
            radical.owms._logger.warn ("cannot read WL description at '%s'" \
        ...
        for parser in self._parsers :
            radical.owms._logger.debug ("trying parser %s" % parser.name)

            if True :
          # try :
                task_descriptions, relation_descriptions = parser.parse (workload_description)
          # except Exception as e :
          #     radical.owms._logger.warn ("parser %s failed: %s" % (parser.name, e))
          # else :
          #     # success!
          #     break
        ...
        workload = radical.owms.Workload (self.session, 
                                  task_descriptions,
    ...
    def create_workload (self, task_descriptions=None,
        ...
        . This methods breaks the design choice of having the planner as the
          entry point to radical.owms - i.e. the interface with the application layer.
        ...
        workload = radical.owms.Workload (task_descriptions, relation_descriptions)
    ...
    def translate_workload (self, workload_id, overlay_id=None) :
        ...
        workload = radical.owms.WorkloadManager.get_workload (workload_id, _manager=self)
        overlay  = radical.owms.OverlayManager .get_overlay  (overlay_id,  _manager=self)
    ...
    def bind_workload (self, workload_id, overlay_id, bind_mode=None) :
        ...
        `bind_mode` parameter, which can be set to `radical.owms.EARLY` or
        `radical.owms.LATE`.
    ...
    def dispatch_workload (self, workload_id, overlay_id) :
        ...
        for (task_id, task) in workload.tasks.iteritems() :
            for (unit_id, unit) in task.units.iteritems() :
                ...
                # get radical.owmss idea of resource configuration
                pilot_id     = unit.pilot_id
                pilot        = radical.owms.Pilot (self.session, pilot_id)
    ...
    def stage_in_workload (self, workload_id) :
        ...
        if  not self._stager :
            self._stager = radical.owms.DataStager (self.session)
