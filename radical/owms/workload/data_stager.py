...
class DataStager (object) :
    ...
    def _parse_staging_directive (self, txt) :
        ...
        in radical.owms previously -- any strings which do not contain staging operators
    ...
    def _stage_in_unit (self, unit) :
        ...
        pilot    = radical.owms.Pilot (unit.session, unit.pilot_id)
        resource = pilot.resource

        # fix the resource placeholders in the unit descriptions.  Get the
        # radical.owms
        ...
        for fin in unit.inputs :
            ...
            radical.owms._logger.info ("staging_in %s < %s / %s / %s" \
    ...
    def _stage_in_file (self, src, resource, workdir, tgt, username=None) :
        ...
        radical.owms._logger.debug ('copy %s -> %s / %s' % (src_url, resource_url, tgt))
    ...
    def _stage_out_unit (self, unit) :
        ...
        for fout in unit.outputs :
            ...
            radical.owms._logger.info ("staging_out %s < %s / %s / %s" \
                                         %  (one, pilot.resource, workdir, two))
    ...
    def _stage_out_file (self, tgt, resource, srcdir, src, username=None) :
        ...
        radical.owms._logger.debug ('copy %s <- %s' % (tgt_url, src_url))
