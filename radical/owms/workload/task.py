...
class Task (tu.Properties, tu.Timed) :
    ...
    an application, and is part of a workload managed by radical.owms.
    ...
    def __init__ (self, session, descr, _workload=None) :
        ...
        tu.Timed.__init__            (self, 'radical.owms.Task', self.id)
        self.session.timed_component (self, 'radical.owms.Task', self.id)

        if  self.workload :
            self.workload.timed_component (self, 'radical.owms.Task', self.id)
    ...
    def cancel (self) :
        ...
        if  self.state in [DISPATCHED] :
            ...
            radical.owms._logger.info ('cancel task     %s' % self.id)
    ...
    def _add_unit (self, cu_descr) :
        ...
        if  not isinstance (cu_descr, radical.owms.ComputeUnitDescription) :
            raise TypeError ("expected ComputeUnitDescription, got %s" % type(cu_descr))

        unit = radical.owms.ComputeUnit (self.session, cu_descr, _task=self)

        self.units[unit.id] = unit
        self.timed_component (unit, 'radical.owms.Unit', unit.id)
    ...
    def get_state (self) :
        ...
        radical.owms._logger.debug ('task state %-6s : %-10s %s' % (self.id, self.state, str(unit_states)))
