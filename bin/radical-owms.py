...
. Uses RADICAL-OWMS >= 0.1

Assumptions:

TODOs:
. Move the workload/task classes to a dedicated tool/module.
. Generalize RADICAL-OWMS instantiation, workload translation and execution to methods.
...
def main(args):
    ...
    # Translate the workload(s) into OWMS internal workload description.
    # NOTE: This is missing in OWMS at the moment. We need a plugin for each
    #       workload.
    for w in aimes_workloads:
        for t in w.tasks:

            cu_description = owms.TaskDescription()
            ...
            # Comment out until data staging will not be supported by OWMS.
    ...
    # Create a session for OWMS.
    session = owms.Session(
        {
            'planner_concurrent': {
    ...
    # Instantiate OWMS planner and managers.
    planner = owms.Planner(planner = args.owms_planner, session = session)

    # if args.data_staging:
    #     workload_manager = owms.WorkloadManager(dispatcher  = args.owms_workload_dispatcher,
    #                                             session     = session)
    # else:
    #     workload_manager = owms.WorkloadManager(dispatcher  = args.owms_workload_dispatcher,
    #                                             session     = session)
    workload_manager = owms.WorkloadManager(dispatcher  = args.owms_workload_dispatcher,
                                            scheduler   = args.owms_workload_scheduler,
                                            session     = session)

    overlay_manager  = owms.OverlayManager (scheduler   = args.owms_overlay_scheduler,
                                            provisioner = args.owms_overlay_provisioner,
                                            session     = session)
    ...
    owms.execute_workload(workload_id, planner, overlay_manager,
        workload_manager, strategy=args.owms_strategy)
...
class Task(object):
    ...
    def write_executable(self):
        ...
        # TODO: Change the directory once the data staging mess in OWMS will
        # be fixed.
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        metavar = 'owms_strategy',
        help    = 'The strategy used by OWMS in order to execute the given \
    ...
    parser.add_argument(
        ...
        default = 'concurrent',
        metavar = 'owms_planner',
        help    = 'The planner used by OWMS. Default: concurrent'
    )

    parser.add_argument(
        '-ews', '--owms-workload-scheduler',
        choices = ['first', 'round_robin', 'ttc_load_balancing'],
        default = 'round_robin',
        metavar = 'owms_workload_scheduler',
        help    = 'The algorithm used to schedule the workload on the targeted \
        resources. Default: round_robin.'
    )

    parser.add_argument(
        '-ewd', '--owms-workload-dispatcher',
        choices = ['local', 'bigjob_pilot', 'sinon'], default='bigjob_pilot',
        metavar = 'owms_workload_dispatcher',
        help    = 'The dispatcher used by OWMS. Default: local'
    )

    parser.add_argument(
        '-eos', '--owms-overlay-scheduler',
        choices = ['local', 'round_robin'],
        default='round_robin',
        metavar = 'owms_overlay_scheduler',
        help    = 'The algorithm used to schedule the overlay on the targeted \
        resources. Default: round_robin.'
    )

    parser.add_argument(
        '-eop', '--owms-overlay-provisioner',
        choices = ['local', 'bigjob_pilot', 'sinon'], default='bigjob_pilot',
        metavar = 'owms_overlay_provisioner',
    ...
    if args.execution_mode == 'local':
        args.owms_workload_dispatcher = 'local'
        args.owms_overlay_provisioner = 'local'

    if args.pilot_system == 'sinon':
        args.owms_workload_dispatcher = 'sinon'
        args.owms_overlay_provisioner = 'sinon'

    if args.pilot_system == 'bigjob':
        args.owms_workload_dispatcher = 'bigjob_pilot'
        args.owms_overlay_provisioner = 'bigjob_pilot'
